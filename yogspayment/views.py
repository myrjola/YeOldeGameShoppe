...
def success(request):
	...
	elif GameLicense.objects.filter(game_id = game.id, player_id = user.id).count() == 0:
		boughtgame = GameLicense(game_id = game.id, player_id = user.id, price = game.price)
		boughtgame.save()
	...
	return render(request, "success.html",{'booughtgame': booughtgame})
